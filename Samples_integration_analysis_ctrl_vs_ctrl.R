...
table(Idents(Integ_data_clus1), Integ_data_clus$type)

group_dist = as.data.frame(table(Idents(Integ_data_clus1), Integ_data_clus1$type))


#Write results into a file
write.table(group_dist,file="EB_encap_ctrl_cell_dis.txt", sep='\t',  quote = F,row.names = FALSE)
...
#Write results into a file
write.table(group_dist,file="EB_encap_ctrl_cell_dis_proporton.txt", sep='\t',  quote = F,row.names = FALSE)
