...
data_EB_2D <- Read10X(data.dir = "E2_S2/filtered_feature_bc_matrix/")

#Create Seurat Object
seurat_ob_S1_EB_2D <- CreateSeuratObject(counts = data_EB_2D, project = "E2_S2", min.cells = 3, min.features = 200)
...
seurat_ob_S1_EB_2D[["percent.mt"]] <- PercentageFeatureSet(seurat_ob_S1_EB_2D, pattern = "^MT-")
...
VlnPlot(seurat_ob_S1_EB_2D, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
#dev.off()

#Load data
data_Encap_2D <- Read10X(data.dir = "E5_S5/filtered_feature_bc_matrix/")

#Create Seurat Object
seurat_ob_S5_Encap_2D  <- CreateSeuratObject(counts = data_Encap_2D, project = "E6_S6", min.cells = 3, min.features = 200)

head(seurat_ob_S5_Encap_4D@meta.data, 5)

# To check the content of dead cells, we can check mitocondrial percentage; The [[ operator can add columns to object metadata. This is a great place to stash QC stats
seurat_ob_S5_Encap_2D[["percent.mt"]] <- PercentageFeatureSet(seurat_ob_S5_Encap_2D, pattern = "^MT-")

# Show QC metrics for the first 5 cells
head(seurat_ob_S5_Encap_2D@meta.data, 5)

#jpeg('QC_EB_2D.jpg', units="in", width=10, height=8, res=300)
# Visualize QC metrics as a violin plot
VlnPlot(seurat_ob_S5_Encap_2D, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
#dev.off()

#Add Desired columns to the Seurat object
#Sample Type
seurat_ob_S1_EB_2D$type <- "EB_2D"
seurat_ob_S5_Encap_2D$type <- "Encap_2D"

# filter genes with low values (Here, we are removing empty cells/droplet)
EB_2D <- subset(seurat_ob_S1_EB_2D, subset = nFeature_RNA > 250)
Encap_2D <- subset(seurat_ob_S5_Encap_2D, subset = nFeature_RNA > 250)

#normalize data
EB_2D_norm<- NormalizeData(EB_2D, verbose = FALSE)
Encap_2D_norm<- NormalizeData(Encap_2D, verbose = FALSE)
...
Encap_2D_ftr<- FindVariableFeatures(Encap_2D_norm, selection.method = "vst", nfeatures = 5000)

#let's first analyze each individual sample
# Convert seurat object to monocle dataset
EB_4D_cds <- as.cell_data_set(EB_4D_ftr)
Encap_4D_cds <- as.cell_data_set(Encap_4D_ftr)
...
plot_cells(EB_4D_cds_clus, color_cells_by="type") +
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")

plot_cells(EB_4D_cds_clus, color_cells_by="partition") +
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")


EB_4D_mat <- as(as.matrix(GetAssayData(seurat_ob_S1_EB_4D, assay = "RNA")),  'sparseMatrix')

EB_4D_mat

#create metadata dataframe 
pd_EB_4D <- data.frame(seurat_ob_S1_EB_4D@meta.data)
head(pd_EB_4D )
#keep important columns in metadata that are relevant 
pData_EB_4D <- pd_EB_4D %>% select(nCount_RNA, nFeature_RNA, type)

#create features dataframe
fData_EB_4D <- data.frame(gene_short_name = row.names(EB_4D_mat), row.names = row.names(EB_4D_mat))

#Construct monocle dataset
monocle_EB_4D <- new_cell_data_set(expression_data = EB_4D_mat, cell_metadata = pData_EB_4D, gene_metadata = fData_EB_4D)

monocle_EB_4D

#preprocess - Linear dimensionality reduction
monocle_EB_4D_p = preprocess_cds(monocle_EB_4D, num_dim = 50)

#PCA plot
plot_pc_variance_explained(monocle_EB_4D_p)
...
#Non-linear dimensionality reduction - UMAP
monocle_EB_4D_umap <- reduce_dimension(monocle_EB_4D_p)

#scatterplot uMAP
plot_cells(monocle_EB_4D_umap, label_groups_by_cluster=FALSE,  color_cells_by = "type")+
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")
...
monocle_EB_4D_cluster <- cluster_cells(monocle_EB_4D_umap)
...
###################### Encap ##################
Encap_4D_mat <- as(as.matrix(GetAssayData(seurat_ob_S6_Encap_4D, assay = "RNA")),  'sparseMatrix')

#create metadata dataframe 
pd_Encap_4D <- data.frame(seurat_ob_S6_Encap_4D@meta.data)

#keep important columns in metadata that are relevant 
pData_Encap_4D <- pd_Encap_4D %>% select(nCount_RNA, nFeature_RNA, type)

#create features dataframe
fData_Encap_4D <- data.frame(gene_short_name = row.names(Encap_4D_mat), row.names = row.names(Encap_4D_mat))

#Construct monocle dataset
monocle_Encap_4D <- new_cell_data_set(expression_data = Encap_4D_mat, cell_metadata = pData_Encap_4D, gene_metadata = fData_Encap_4D)


#preprocess - Linear dimensionality reduction
monocle_Encap_4D_p = preprocess_cds(monocle_Encap_4D, num_dim = 50)

#PCA plot
plot_pc_variance_explained(monocle_Encap_4D_p)
...
#Non-linear dimensionality reduction - UMAP
monocle_Encap_4D_umap <- reduce_dimension(monocle_Encap_4D_p)

#scatterplot uMAP
plot_cells(monocle_Encap_4D_umap, label_groups_by_cluster=FALSE,  color_cells_by = "type")+
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")

#Cluster your cells: each cell is assigned not only to a cluster but also to a partition. When you are learning trajectories, each partition will eventually become a separate trajectory. We run cluster_cells()as before.
monocle_Encap_4D_cluster <- cluster_cells(monocle_Encap_4D_umap)

plot_cells(monocle_Encap_4D_cluster, color_cells_by = "cluster") +
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")

#partition based
plot_cells(monocle_Encap_4D_cluster, color_cells_by = "partition") +
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")
...
marker_genes <- markers_neuroblast       #### change marker genes according to cells we are looking for

plot_cells(monocle_EB_4D_cluster,  
           min_expr= 1.5, 
           norm_method = "log",
...
#Encap
plot_cells(monocle_Encap_4D_cluster,  
           min_expr= 1.5, 
           norm_method = "log",
           label_groups_by_cluster=FALSE, 
           genes=marker_genes,
...
#keep important columns in metadata that are relevant 
pData_ctrl <- pd_ctrl %>% select(orig.ident, nCount_RNA, nFeature_RNA, type)
...
#Cluster your cells: each cell is assigned not only to a cluster but also to a partition. When you are learning trajectories, each partition will eventually become a separate trajectory. We run cluster_cells()as before.
monocle_cluster1_ctrl <- cluster_cells(monocle_umap1_ctrl)

#type based
plot_cells(monocle_cluster1_ctrl, color_cells_by = "type") +
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")

#Clusters based
plot_cells(monocle_cluster1_ctrl, color_cells_by = "cluster") +
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")


#Learn the trajectory graph: we will fit a principal graph within each parition using the learn_graph() function:
monocle_traj_ctrl<- learn_graph(monocle_cluster1_ctrl,  use_partition = FALSE)

jpeg('Integrated_trajectory_type.jpg', units="in", width=10, height=8, res=300)

plot_cells(monocle_traj_ctrl, color_cells_by = "type",
           label_groups_by_cluster=FALSE,
           label_leaves=FALSE,label_branch_points=FALSE)+
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")
...
plot_cells(monocle_traj_ctrl, color_cells_by = "cluster",
           label_groups_by_cluster=FALSE,
           label_leaves=FALSE,label_branch_points=FALSE)+
  theme(legend.text=element_text(size=6)) + #set the size of the text
  theme(legend.position="right")
...
##### Order cells trajectoty based ###

colData(monocle_traj_ctrl)$cluster <- as.character(clusters( monocle_traj_ctrl))

cluster_table <- table(round(pseudotime(monocle_traj_order1),2), monocle_traj_order1$type)
write.table(cluster_1_table,file="cluster_1_table_pseudotime.txt", sep='\t',  quote = F,row.names = T)
cluster_1_table


monocle_traj_order_ctrl <- order_cells(monocle_traj_ctrl)
